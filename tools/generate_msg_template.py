    ...
    def Update(self, Msg):
        if self.type in Primitivelist:
            ...
            if self.pkg + "/" + self.type in UserTypeList:
                self.UType = "F_" + \
                    UserTypeList[self.pkg + "/" + self.type].GeneratedName
                ...
                self.UType = "F_" + \
                    UserTypeList["std_msgs" + "/" + self.type].GeneratedName
...
class MsgType:
    ...
    def Update(self):
        for k in self.props:
            k.Update(self)
        print("Update Type {0} as F_{1}".format(
            self.namefull, self.GeneratedName))
        print(self)
...
def AnalyseMsgStr(pkg, fstr, name):
    ...
    constantdefs = re.findall(
        "^\s*(?!string)([\w/]+)\s+(\w+)\s*=\s*(\w+)$", fstr, re.M)
    constantdefstr = re.findall(
        "^\s*(string)\s+(\w+)\s*=\s*(.*?)\s*$", fstr, re.M)
    print(fstr)
    print(defs)
    MsgList = [MsgField(v, k, pkg) for k, v in defs]
    MsgList.extend([MsgField(v, k, pkg, Array=True) for k, v in defsArray])
    MsgList.extend([MsgField(v, k, pkg, Constant=True, ConstantField=c)
                    for k, v, c in constantdefs])
    MsgList.extend([MsgField(v, k, pkg, Constant=True,
                             ConstantField='"' + c + '"') for k, v, c in constantdefstr])
...
def TopologicalSorting(msgs):
    ...
    print("Success Topoligical Sorting : {0}".format(
        len(NoReferenced) == TotalNum))
    return NoReferenced


def AnalyseMsgDirectory(path, headerName, sourceName):
    ros_pkgs = next(os.walk(path))[1]
    print("Analyse {0}".format(ros_pkgs))
    import glob
    MsgList = set()
    for pkg in ros_pkgs:
        msgs = map(lambda path: AnalyseMsgFile(pkg, path),
                   glob.glob(path + '/' + pkg + "/*.msg"))
        MsgList = MsgList.union(msgs)
    for msg in MsgList:
        msg.Update()
    MsgListRight = TopologicalSorting(MsgList)
    #print MsgList
    fstr = open("FStructTemplate.h.template").read()
    fstrpp = open("FStructTemplate.cpp.template").read()

    from jinja2 import Template
    headerTemplate = Template(fstr)
    sourceTemplate = Template(fstrpp)
    ...
    headerFile.write(headerTemplate.render(**rdict))
    sourceFile.write(sourceTemplate.render(**rdict))
...
if __name__ == "__main__":
    ...
    headerName = "../Source/Unreal_ROS/Classes/ros_msg_test.h"
    sourceName = "../Source/Unreal_ROS/Private/ros_msg_test.cpp"

    print(AnalyseMsgDirectory("data", headerName, sourceName))
